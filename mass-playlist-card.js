 ...
 * Diseñada para la vista Sections de Home Assistant.
 *
 * Instalación:
 *   1. Copia a /config/www/mass-playlist-card.js
...
// order_by values que expone HA (obtenidos de la UI de Developer Tools)
const ORDER_BY_OPTIONS = [
  { value: 'timestamp_added_desc', label: 'Añadido (desc)'                   },
  { value: 'timestamp_added',      label: 'Añadido'                           },
  { value: 'last_played_desc',     label: 'Última reproducción (desc)'        },
  { value: 'last_played',          label: 'Última reproducción'               },
  { value: 'play_count_desc',      label: 'Número de reproducciones (desc)'   },
  { value: 'play_count',           label: 'Número de reproducciones'          },
  { value: 'random',               label: 'Aleatorio'                         },
  { value: 'random_less_played',   label: 'Aleatorio + menos reproducido'     },
  { value: 'name',                 label: 'Nombre'                            },
  { value: 'name_desc',            label: 'Nombre (desc)'                     },
  { value: 'sort_name',            label: 'Ordenar nombre'                    },
  { value: 'sort_name_desc',       label: 'Ordenar nombre (desc)'             },
  { value: 'year_desc',            label: 'Año (desc)'                        },
  { value: 'year',                 label: 'Año'                               },
  { value: 'artist_name',          label: 'Nombre del artista'                },
  { value: 'artist_name_desc',     label: 'Nombre del artista (desc)'         },
...
class MassPlaylistCardEditor extends HTMLElement {
  ...
  set hass(hass) {
    this._hass = hass;
    // Propagar hass a los ha-entity-picker del DOM
  ...
  _renderEditor() {
    ...
    shadow.innerHTML = `
      ...
      ${isManual ? `
        <div class="section-title">Items manuales (arrastra para reordenar)</div>
    ...
    // ── ha-form para los campos principales ──
    ...
    form.schema = [
      {
        name: 'entity_id',
        required: true,
        label: 'Altavoz de Music Assistant',
      ...
      {
        name: 'item_size',
        label: 'Tamaño de portada (columnas)',
    ...
    form.addEventListener('value-changed', (e) => {
      ...
      // Si cambia order_by a manual, inicializar array si no existe
      ...
      // Re-render para mostrar/ocultar sección manual
    ...
    // ── Inicializar inputs de texto para URIs manuales ──
    if (isManual) {
...
class MassPlaylistCard extends HTMLElement {
  constructor() {
    ...
    this._hass           = null;
    this._config         = {};
    this._items          = [];   // items resueltos (librería o manual)
    this._loading        = false;
    this._error          = null;
  ...
  setConfig(config) {
    if (!config.entity_id) throw new Error('mass-playlist-card: Se requiere entity_id');
    this._config = { media_type: 'playlist', order_by: 'timestamp_added_desc', item_size: 3, manual_items: [], ...config };
  ...
  async _fetchItems() {
    ...
    // Modo manual: resolver URIs directamente via search
    ...
    if (!configEntryId) {
      this._error = 'No se encontró la instancia de Music Assistant.';
      this._loading = false;
      this._updateGrid();
      return;
    }
    ...
    try {
      ...
      this._error = err.message || 'Error al cargar contenido';
    }
  ...
  async _resolveManualItems() {
    ...
    // Resolvemos cada URI buscándola en la librería por su URI exacta
    // MA acepta search con el URI completo y devuelve el item
    const resolved = [];
  ...
  _updateGrid() {
    ...
    if (this._loading) { root.innerHTML = `<div class="state"><div class="loader"></div>Cargando…</div>`; return; }
    if (this._error)   { root.innerHTML = `<div class="state">⚠️ ${this._error}</div>`; return; }
    if (!this._items.length) { root.innerHTML = `<div class="state">No hay contenido en la librería</div>`; return; }
    ...
    const rows = this._calcRows(w, h, cols);
    ...
    for (const item of items) {
      ...
      el.setAttribute('aria-label', `Reproducir ${name}`);
      el.title = name;
...
window.customCards.push({
  ...
  name:        'Music Assistant — Media Grid',
  description: 'Grid de portadas de Music Assistant para Sections view',
  preview:     false,
});
